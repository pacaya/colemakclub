...
/*__________________progress tracking vars___________________*/

// Progress tracking global variables
var progressData = null; // current layout's progress object
var globalGoals = null; // shared goals across layouts {wpm, accuracy}
var levelLockingEnabled = true; // master switch for level limiting

// Progress tracking DOM elements (initialized after DOM ready)
var goalWpmInput,
  goalAccuracyInput,
  levelLockingToggle,
  unlockAllButton,
...
function initProgressData(layoutName) {
  var data = {
    layout: layoutName,
  ...
  // Initialize stats for all 8 levels
  for (var i = 1; i <= 8; i++) {
    data.levelStats[i] = {
...
function loadProgressData() {
  var key = "progressData_" + currentLayout;
...
// Save current progress data to localStorage
function saveProgressData() {
  var key = "progressData_" + currentLayout;
  localStorage.setItem(key, JSON.stringify(progressData));
}

// Load global goals from localStorage
function loadGlobalGoals() {
  var stored = localStorage.getItem("globalGoals");
  if (stored) {
    try {
      globalGoals = JSON.parse(stored);
    } catch (e) {
      globalGoals = { wpm: 50, accuracy: 98 };
    }
  } else {
    globalGoals = { wpm: 50, accuracy: 98 };
  }

  // Load level locking preference
  var lockingStored = localStorage.getItem("levelLockingEnabled");
  levelLockingEnabled =
    lockingStored === null ? true : lockingStored === "true";
}

// Save global goals to localStorage
function saveGlobalGoals() {
  localStorage.setItem("globalGoals", JSON.stringify(globalGoals));
}

// Check if a test result meets the goals
function meetsGoals(wpm, accuracy) {
  return wpm >= globalGoals.wpm && accuracy >= globalGoals.accuracy;
}

// Calculate statistics from test history array
...
function recordTestResult(level, wpm, accuracy) {
  ...
  // Check if should auto-advance (3 consecutive successes and not at max level)
  if (
    levelData.consecutiveSuccesses >= 3 &&
    level < 8 &&
    level >= progressData.unlockedLevel
  ) {
    return true;
  }
...
function unlockNextLevel(fromLevel) {
  var nextLevel = parseInt(fromLevel) + 1;
  if (nextLevel <= 8 && nextLevel > progressData.unlockedLevel) {
    progressData.unlockedLevel = nextLevel;
    // Reset consecutive successes for the completed level
    progressData.levelStats[fromLevel].consecutiveSuccesses = 0;
    saveProgressData();
  }
}
...
function isLevelUnlocked(level) {
  if (!levelLockingEnabled) return true;
  return parseInt(level) <= progressData.unlockedLevel;
}

// Update level button states based on progress
...
// Unlock all levels (bypass level locking)
function unlockAllLevels() {
  progressData.unlockedLevel = 8;
  saveProgressData();
  updateLevelLocking();
}
...
// Show the level advancement popup
function showLevelAdvancementPopup(fromLevel, toLevel) {
  if (!progressionModal) return;

  var fromLevelName = getLevelName(fromLevel);
  var toLevelName = getLevelName(toLevel);

  progressionMessage.innerHTML =
  ...
  // Store the target level for the button handlers
  progressionModal.dataset.fromLevel = fromLevel;
  progressionModal.dataset.toLevel = toLevel;
}

// Hide the progression popup
function hideProgressionPopup() {
  if (progressionModal) {
...
// Reset all statistics for current layout
function resetProgressStats() {
  if (
    confirm(
      "Reset all statistics for " + currentLayout + "? This cannot be undone.",
    )
  ) {
    progressData = initProgressData(currentLayout);
    saveProgressData();
    ...
    // If level locking is enabled, go back to level 1
    if (levelLockingEnabled && parseInt(currentLevel) > 1) {
      switchLevel(1);
    }
  }
}

// Initialize progress tracking DOM elements and event listeners
function initProgressTracking() {
  // Get DOM elements
  goalWpmInput = document.querySelector("#goalWpmInput");
  goalAccuracyInput = document.querySelector("#goalAccuracyInput");
  levelLockingToggle = document.querySelector("#levelLockingToggle");
  ...
  advanceToNextLevelBtn = document.querySelector("#advanceToNextLevel");

  // Set initial values for goal inputs
  if (goalWpmInput) {
    goalWpmInput.value = globalGoals.wpm;
    goalWpmInput.addEventListener("change", function () {
      var val = parseInt(goalWpmInput.value);
      if (val >= 10 && val <= 200) {
        globalGoals.wpm = val;
        saveGlobalGoals();
      } else {
  ...
  if (goalAccuracyInput) {
    goalAccuracyInput.value = globalGoals.accuracy;
    goalAccuracyInput.addEventListener("change", function () {
      var val = parseInt(goalAccuracyInput.value);
      if (val >= 50 && val <= 100) {
        globalGoals.accuracy = val;
        saveGlobalGoals();
      } else {
        goalAccuracyInput.value = globalGoals.accuracy;
      }
  ...
  if (levelLockingToggle) {
    levelLockingToggle.checked = levelLockingEnabled;
    levelLockingToggle.addEventListener("change", function () {
      levelLockingEnabled = levelLockingToggle.checked;
      localStorage.setItem("levelLockingEnabled", levelLockingEnabled);
      updateLevelLocking();

      // If enabling locking and current level is now locked, switch to highest unlocked level
      if (levelLockingEnabled && !isLevelUnlocked(currentLevel)) {
        switchLevel(progressData.unlockedLevel);
      }
    });
  }
  ...
  if (advanceToNextLevelBtn) {
    advanceToNextLevelBtn.addEventListener("click", function () {
      var toLevel = parseInt(progressionModal.dataset.toLevel);
...
layout.addEventListener("change", (e) => {
  ...
  // Load progress data for the new layout
  loadProgressData();
  updateLevelLocking();

  // If current level is locked in new layout, switch to level 1
  if (!isLevelUnlocked(currentLevel)) {
    currentLevel = 1;
    localStorage.setItem("currentLevel", currentLevel);
    // Update level button UI
    clearCurrentLevelStyle();
    document.querySelector(".lvl1").classList.add("currentLevel");
  }
...
function switchLevel(lev) {
  // Check if level is locked (but allow if progressData isn't loaded yet during init)
  if (progressData && !isLevelUnlocked(lev)) {
  ...
  // take care of styling for the cheatsheet (use wordLevel for level 8)
  updateCheatsheetStyling(wordLevel);
}
...
function endGame() {
  ...
  // Check if should show advancement popup
  if (shouldAdvance && parseInt(currentLevel) < 8) {
    var nextLevel = parseInt(currentLevel) + 1;
    // Unlock the next level BEFORE showing popup (so user can access it even if they stay)
